...
export type TrnsBulkEditResult = {
  changedIds: TrnId[]
...
export function buildTrnsBulkEdit({
  action,
  ids,
  ...
  const result: TrnsBulkEditResult = {
    changedIds: [],
    ineligible: [],
    unchangedIds: [],
    values: {},
...
export function buildTrnsBulkEdits({
  ...
  const ineligible: TrnsBulkEditResult['ineligible'] = []
  const values: Record<TrnId, TrnItem> = {}
  let currentItems = items

  for (const action of actions) {
    const step = buildTrnsBulkEdit({
      action,
      ids,
      isCategoryTransactible,
      isWalletSelectable,
