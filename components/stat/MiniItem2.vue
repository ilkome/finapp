...
import type { MoneyTypeSlug, MoneyTypeSlugSum } from '~/components/stat/types'
...
const emit = defineEmits<{
  onChangeNetTab: [t: MoneyTypeSlugSum]
}>()
...
const periodsEmptyTrnsIds = computed(() =>
  getPeriodsWithEmptyTrnsIds({
    ...
    period: period.value,
    until: dayjs().endOf('d').valueOf(),
  }),
)

const periodsWithTrnsIds = computed(() => getPeriodsWithTrns(props.selectedTrnsIds, period.value, periodsEmptyTrnsIds.value))
const periodsWithTrnsIds2 = computed(() => getPeriodsWithTrns(props.trnsIds, period.value, periodsEmptyTrnsIds.value))
...
const groupedTrnsTotals2 = computed(() =>
  Object.keys(periodsWithTrnsIds2.value)
    .sort((a, b) => +a - +b)
    .reduce(
      (acc, date) => {
        acc[date] = getTotalOfTrnsIds(periodsWithTrnsIds2N.value[date])
        return acc
      },
      {} as Record<string, TotalReturns>,
    ),
...
const series = computed(() => getSeries(groupedTrnsTotals.value, props.netTab))
const selectedTrnsIdsForTrnsList = computed(
  () => periodsWithTrnsIds.value[date.value],
)
const totals = computed(() =>
  getTotalOfTrnsIds(periodsWithTrnsIds2.value[date.value]),
)
...
function setPeriodAndDate(periodName: PeriodNameWithoutAll) {
  ...
  date.value = dayjs(categories.value.at(-1)).valueOf()
...
<template>
  <div class="grid grid-rows-[auto,1fr] h-full overflow-hidden">
    <!-- Date -->
    <div class="grid gap-3 z-10 py-2 px-3 sticky top-0 bg-item-9">
      <div class="flex grow">
        <StatDateNav
    ...
    <div ref="sliderRef" class="swiper-container overflow-hidden h-full">
      <div class="swiper-wrapper h-full">
        <div class="swiper-slide h-full overflow-y-auto">
          <div class="grid gap-3 -px-3">
            <!-- <div class="grid content-center place-items-center pt-2">
              <StatSum3
                :amount="totals.sum"
                class=""
                isTotal
                type="sum"
                @click="emit('onChangeNetTab', 'sum')"
              />
            </div> -->
            ...
            <div class="w-full overflow-hidden overflow-y-auto">
              <div class="flex justify-stretch flex-wrap bg-item-4 -rounded-lg -[contain:paint]">
                ...
                <StatSum3
                  ...
                  :class="{
                    'bg-item-5': props.netTab === 'expense',
                  }"
                  class="p-3 flex-grow"
                  isTotal
                  type="expense"
                  @click="emit('onChangeNetTab', 'expense')"
                />
                <StatSum3
                  :amount="totals.income"
                  :class="{
                    'bg-item-5': props.netTab === 'income',
                  }"
                  class="p-3 flex-grow"
                  isTotal
                  type="income"
                  @click="emit('onChangeNetTab', 'income')"
                />
              </div>
            </div>
          </div>
