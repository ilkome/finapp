...
import type { CategoryId, TrnId, TrnItem, Trns } from '~/components/trns/types'
...
export const useTrnsStore = defineStore('trns', () => {
  ...
  const lastCreatedTrnId = computed(() => {
    ...
    const transferCategoriesIds = getTransferCategoriesIds(categoriesStore.items)
  ...
  const firstCreatedTrnId = computed<TrnItem | false>(() => {
    ...
    return Object.keys(items.value)
      .sort((a, b) => items.value[a].date - items.value[b].date)[0]
  })
  ...
  const selectedTrnsIds = computed(() => {
    ...
    const categoriesIds: CategoryId[] | false = filterStore.catsIds.length > 0
  ...
  const firstCreatedTrnIdFromSelectedTrns = computed(() => selectedTrnsIds.value[selectedTrnsIds.value.length - 1])
  ...
  const selectedTrnsIdsWithDate = computed(() => {
    ...
    const startDateValue = filterDate.startOf(filterStore.periodWithoutAll).valueOf()
    const endDateValue = filterDate.endOf(filterStore.periodWithoutAll).valueOf()
  ...
  function addTrn({ id, values }) {
    ...
    localforage.setItem('finapp.trns', { ...items.value, [id]: valuesWithEditDate })

    setTrns({ ...items.value, [id]: valuesWithEditDate })

    saveData(`users/${userStore.uid}/trns/${id}`, valuesWithEditDate)
      .then(() => {
  ...
  function deleteTrn(id: TrnId) {
    ...
    removeData(`users/${userStore.uid}/trns/${id}`)
