...
**Why OPFS on Chromium, IndexedDB elsewhere:**
`OPFSWriteAheadVFS` needs no `SharedArrayBuffer` or COOP/COEP headers (it runs in a dedicated worker with OPFS sync access handles), supports parallel read connections, and made the cold start ~40 % faster on an Android phone. It relies on `readwrite-unsafe` access handles, which only Chromium 121+ has, so iOS and Firefox keep `IDBBatchAtomicVFS`. See [Speed Update](/en/reference/speed-update-2026-09).
