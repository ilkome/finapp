...
## Client-generated UUIDs (no `local_` prefix, no remapping)

Every new entity gets a UUID generated on the client (`crypto.randomUUID()` or equivalent) before the local write. That UUID is the permanent ID - there is no temporary `local_` prefix and no remapping after server acknowledgement.

**Why no `local_` IDs:**
The previous backend used a temporary `local_abc123` prefix, then remapped to a server-assigned ID after the mutation succeeded. This required tracking pending ID remaps across all stores and cascading updates (e.g., trns referencing a newly-created wallet). With PowerSync, the client owns the ID permanently: Supabase schema uses `text` PKs with `default gen_random_uuid()::text`, so whatever UUID the client generates is accepted as-is. No remapping, no cascade complexity.
...
- [Validation Strategy](/en/development/validation-strategy) - client-only validation with Zod
