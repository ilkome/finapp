...
- **Session lost involuntarily (token revoked / expired server-side).** Surfaces on reconnect as a genuine `SIGNED_OUT`. The auth watcher (it watches `[uid, isAuthReady]`, so a cold start whose persisted session is already dead is covered too) then **pauses** sync (`pausePowerSync` - `db.disconnect()`, no clear) instead of wiping, keeping local SQLite **and the unsynced queue**. When pending writes exist, a re-auth toast is shown (`sync.errors.sessionLostPending`); re-authenticating as the same user reconnects and drains the queue. Only an **explicit** sign-out (`useUserStore.signOut()`) wipes local data via `disconnectPowerSync` → `disconnectAndClear`.

## Sign-out and the upload queue

`disconnectAndClear` destroys the upload queue, so `signOut()` first waits for it to drain (`waitForUploadsDrained`, up to 8 s) and **refuses to sign out** while ops remain (offline, or an upload that keeps failing), showing `sync.errors.signOutPending`. A wipe that still discards queued ops - a different user signing in over a paused session, or a user-initiated force resync - reports the count through `setDiscardedPendingHandler` (`sync.errors.pendingDiscarded`) rather than dropping them silently.

## Sync status in the UI

`useSyncStatus` (`app/app/components/app/useSyncStatus.ts`) subscribes once to PowerSync's status plus the synced tables and exposes `connected`, `pending` (queued ops) and `uploadError`. The header menu button shows a dot while any of them is off, and the account panel (`UserViewLogout`) shows the details.
...
Postgres error classes `22xxx` (data exception), `23xxx` (integrity constraint violation), `42501` (RLS - insufficient privilege) and PostgREST request/schema errors `PGRST1xx`/`PGRST2xx` (unknown table or column, malformed body - identical on every retry) are treated as fatal. Anything else, including `PGRST3xx` (JWT) and `PGRST5xx` (connection pool), stays retryable. The failing op **and every op queued after it** are discarded so the queue is not blocked, then auto-reconciled by the plugin (`planDivergence`, `app/services/powersync/uploadReconcile.ts`):
