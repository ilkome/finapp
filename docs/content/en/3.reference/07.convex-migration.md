...
description: What changed between v6 (Firebase) and v7 (Convex). Convex was later replaced by Supabase + PowerSync.
navigation:
  icon: i-lucide-hamburger
seo:
  title: Convex Migration - What Changed
  description: Side-by-side comparison of Finapp v6 (Firebase) and v7 (Convex) - backend, performance, security, tests, and architecture. Historical reference.
---

> **Three eras.** Finapp has gone through three backend generations:
> - **v6** - Firebase Realtime Database + Firebase Auth
> - **v7** - Convex (backend-as-code) + Better Auth (described in this document)
> - **current** - Supabase Postgres + PowerSync (offline-first) + Supabase Auth
>
> This document is the historical side-by-side comparison of the Firebase-to-Convex migration. The app-level improvements (performance, refactoring, UX, TypeScript, i18n) described here are still valid and present in the current codebase. The backend, auth, offline queue, and dependency sections reflect the v7 Convex state; see [Architecture](/en/development/architecture) for the current Supabase + PowerSync architecture.

Side-by-side comparison of what the app looks like on the [`firebase`](https://github.com/ilkome/finapp/tree/firebase) branch (v6, Firebase) vs the Convex era (v7).
...
| **Migration** | N/A | `utils/migrate/firebase.js` - CLI to import Firebase JSON exports into Convex |
| **Account deletion** | N/A | `deleteUserDataFull()` - paginated wipe of app + auth tables |

### What Convex gave

- **Typed mutations** - full TypeScript on both sides, auto-generated API types
- **Server-side validation** - string length limits, numeric ranges, field non-emptiness, business rules
...
- **Delta sync** - only changed records fetched after initial load

## Store Pattern

| | firebase | v7 (Convex) |
|---|---|---|
| **Data source** | Firebase listener -> `setTrns()` | Convex query -> `mergeOfflineOps()` -> `set()` |
...
| **ID system** | Firebase push IDs | `local_` prefix -> remapped to Convex ID after mutation |
| **Optimistic UI** | Partial (fire-and-forget) | Full: update UI -> push op -> fire mutation -> remap ID on success |
| **Sync verification** | None | XOR hash (`fnv1a`) of transaction IDs; client vs server hash comparison on every sync |
...
| What | firebase | v7 (Convex) | Impact |
|------|----------|------|--------|
...
_(Historical - describes the v7 Convex era. Current auth: Supabase Auth, email/password only. See [Architecture](/en/development/architecture).)_

| | firebase | v7 (Convex) |
|---|---|---|
| **Provider** | Firebase Auth | Better Auth |
| **Plugin** | `nuxt-vuefire` (auto-handles auth) | `plugins/convex.ts` - manual lifecycle |
| **Token flow** | Firebase SDK internal | `setAuth(fetchToken)` -> `authClient.convex.token()` -> `authReadyPromise` |
...
_(Historical - describes the v7 Convex era. Current offline: PowerSync handles sync automatically. See [Architecture](/en/development/architecture).)_
...
| **Operations** | `saveTrnToAddLaterLocal()`, `removeTrnToDeleteLaterLocal()` | `pushSaveOp()`, `pushDeleteOp()` -> generic for any entity |
...
| **Per-entity replay** | N/A | `replayWalletOps`, `replayCategoryOps`, `replayTrnOps`, `replaySettingsOps` - extracted testable functions |
| **Op validation** | None | `validateOpData()` with Zod schemas - corrupted ops logged and removed |
...
_(The v7 Convex test infrastructure had a dedicated `convex` vitest project running in edge-runtime. Current tests run in `unit` (node) and `store` (happy-dom) projects against PowerSync/Supabase mocks.)_
...
_(Historical - describes what changed from firebase to v7 Convex. For current dependencies see the project `package.json`.)_

| Removed (firebase era) | Added (Convex era) |
|---------|-------|
| `firebase` | `convex` |
...
The Convex era deps (`convex`, `@convex-dev/better-auth`, `better-auth`, `convex-helpers`, `convex-test`) were subsequently removed when the backend migrated to Supabase + PowerSync. `@powersync/web` and `@supabase/supabase-js` were added. `zod`, `es-toolkit`, and `@vueuse/nuxt` remain.
