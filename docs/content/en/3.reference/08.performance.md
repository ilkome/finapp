...
What keeps the cold start fast, and how to measure it. Reference numbers below were taken on a prod build (`pnpm build:app`) with a real account of ~8 000 transactions, desktop hardware, assets served from the service worker - treat them as relative, not absolute.
...
On IndexedDB (iOS, Firefox) the boot watches are **serialized** on the single PowerSync worker connection - their first emissions land stair-stepped (`user_settings` → `rates` → `categories` → `wallets` → `trns`), with the full-table `trns` scan dominating. On Chromium the database is on OPFS with a parallel read connection, which cut the `trns` first emission by ~40 % on an Android phone (see [Speed Update](/en/reference/speed-update-2026-09)). Merging the four small queries would save little; windowing `trns` by date was rejected because wallet totals need full history (see [Technical Decisions](/en/reference/tech-decisions)).
...
- **SW precache is narrowed.** Of the eight emitted wa-sqlite WASM variants, only the two the worker can load are precached: `wa-sqlite.<hash>.wasm` (OPFS) and `wa-sqlite-async.<hash>.wasm` (IndexedDB fallback). Cipher builds need an `encryptionKey` we never pass.
- **Hidden pages are paused.** Kept-alive pages pause their component subtree while hidden (`usePauseWhenInactive`), so an edit costs the same no matter how many pages were visited.
