...
- A singleton `supabase-js` client (`autoRefreshToken`, `persistSession`, `detectSessionInUrl: false`).
- `useSupabaseAuth()` composable exposing reactive `session`, `uid`, `user`, `isAuthReady`, plus `signInWithPassword` / `signUp` / `signOut`.
- Session persists in localStorage and auto-refreshes. `onAuthStateChange` keeps a module-level reactive `session` ref in sync.

Auth method: **email/password only** (no OAuth for the MVP).

### Auth Guard

Auth uses two layers:

1. **Route middleware** (`app/app/middleware/auth.global.ts`): synchronous gate on the `finapp.localAuthUid` cookie (`hasAuthCookie()`), no network call, works offline. Redirects to `/login` (preserving `?redirect=`) when absent; bounces `/login` → `/dashboard` when the cookie is present. Demo mode bypasses this check.
2. **PowerSync plugin** (`app/app/plugins/powersync.client.ts`): watches the Supabase `uid`. Sets the `finapp.localAuthUid` cookie when a session resolves; clears it on sign-out. This is the only place the cookie is written.
...
Hard navigation instead of Vue Router - see [Technical Decisions](/en/development/tech-decisions).
...
- [Sync](/en/development/sync) - how PowerSync syncs data between server and local SQLite
- [Offline-first](/en/development/offline-queue) - offline writes, upload queue, and error handling
