...
Run Vitest once for unit tests:

```bash [Terminal]
pnpm test
```

Use `pnpm test:watch` while developing or `pnpm test:coverage` to collect coverage.
...
| `pnpm test:e2e` | Run the demo-mode E2E suite headless against the local dev server. |
| `pnpm test:e2e:prod` | Build the app and run the production-only E2E smoke test. Used in CI. |
...
| `pnpm --filter @finapp/app exec playwright show-report` | Open the HTML report from the last run (screenshots, traces, videos of failures). |

### Typical Workflow

1. First time only: `pnpm test:e2e:install`
2. Writing tests: `pnpm test:e2e:ui`
3. Full demo sweep: `pnpm test:e2e`
4. Production smoke test: `pnpm test:e2e:prod`
5. Investigating a failure: `pnpm --filter @finapp/app exec playwright show-report`

Headed mode and report viewing use Playwright CLI flags directly, so they do not need dedicated package scripts.
...
In the browser, set that value, clear the `finapp.isDemo` cookie, and reload - the app boots into real PowerSync mode as the test user and the seeded data streams in. Credentials/URL are read from `app/.env.local` (overridable via `E2E_EMAIL` / `E2E_PASSWORD`).
