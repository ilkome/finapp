...
1. Create a Supabase project, link the repo and push the schema:
...
2. Run `supabase/powersync_setup.sql` against the project database (SQL editor or `psql` over the **direct** connection string). It creates the `powersync_role` (REPLICATION + BYPASSRLS) and the `powersync` publication that the PowerSync service replicates from.
