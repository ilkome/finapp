...
The Nuxt SPA is statically hosted (Vercel above, but any static host serving `app/.output/public` after `pnpm build:app` works). The backend services are separate:
...
**Per project, not per repo.** Edge functions and their secrets live in the Supabase project, so every environment (prod, dev/preview) needs its own deploy + secrets. A fresh dev project gets the cron **job** from `db push` but no Vault secrets, and the job then posts to a `null` URL - a silent no-op, with the app showing an empty currency list and no error. Check both halves:

```bash [Terminal]
supabase functions list --project-ref <ref>                     # is fetch-rates deployed?
supabase db query --linked "select name from vault.secrets"     # project_url + anon_key present?
supabase db query --linked "select max(date) from public.rates" # is the table actually filling?
```

To verify the scheduled path end to end (not just the function), run the job body by hand and read the response `pg_net` recorded:

```bash [Terminal]
supabase db query --linked "select net.http_post(url := (select decrypted_secret from vault.decrypted_secrets where name='project_url') || '/functions/v1/fetch-rates', headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name='anon_key')), body := '{}'::jsonb)"
supabase db query --linked "select status_code, left(content,120) from net._http_response order by id desc limit 1"
```
::

### Push notifications (`send-push`)

Web Push needs one VAPID keypair per project - the public half ships to the client, the private half stays a function secret. See [push notifications](/en/reference/push-notifications) for how delivery works.

1. **Generate a keypair** (skip if the project already has one - regenerating invalidates every stored subscription in `push_subscriptions`):

   ```bash [Terminal]
   npx web-push generate-vapid-keys --json
   ```

2. **Function secrets** and deploy:

   ```bash [Terminal]
   supabase secrets set --project-ref <ref> VAPID_PUBLIC_KEY=<public> VAPID_PRIVATE_KEY=<private>
   supabase functions deploy send-push --project-ref <ref>
   ```

3. **Client key.** Put the same public key in `VITE_VAPID_PUBLIC_KEY` for that environment (Vercel env + your local `.env.*`), otherwise devices subscribe under a key the sender can't sign for.

Nothing calls `send-push` on a schedule yet - it is the delivery primitive for future reminders; see the test call in [push notifications](/en/reference/push-notifications#test-send).
...
Five tables are per-user: `categories`, `wallets`, `trns`, `user_settings`, `stat_views`. `rates` is global - **don't copy it** (it's populated by `fetch-rates`). The copy is Postgres -> Postgres (the schema is identical across environments), rewriting `userId` to the destination project's auth uid.
