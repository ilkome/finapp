...
### Push notifications (`send-push`, `send-reminders`)
...
4. **Cron bearer.** `send-reminders-daily` (07:00 UTC, migration `*_schedule_send_reminders.sql`) posts with the **service-role** key - a cross-user batch, so the public anon key is not enough - read from a Vault secret:

   ```bash [Terminal]
   supabase db query --linked "select vault.create_secret('<service-role-key>', 'service_role_key')"
   ```

   Without it the scheduled POST is a silent 401. Same diagnosis commands as for `fetch-rates` above.
