...
- Синглтон-клиент `supabase-js` (`autoRefreshToken`, `persistSession`, `detectSessionInUrl: false`).
- Composable `useSupabaseAuth()` с реактивными `session`, `uid`, `user`, `isAuthReady`, а также `signInWithPassword` / `signUp` / `signOut`.
...
Метод авторизации: **только email/password** (OAuth в MVP не предусмотрен).

### Защита авторизации

Авторизация использует два слоя:

1. **Route middleware** (`app/app/middleware/auth.global.ts`): синхронная проверка cookie `finapp.localAuthUid` (`hasAuthCookie()`), без сетевых запросов, работает офлайн. Перенаправляет на `/login` (сохраняя `?redirect=`) при отсутствии; перенаправляет `/login` → `/dashboard` при наличии cookie. Демо-режим обходит эту проверку.
2. **Плагин PowerSync** (`app/app/plugins/powersync.client.ts`): следит за Supabase `uid`. Устанавливает cookie `finapp.localAuthUid` при разрешении сессии; очищает при выходе. Это единственное место, где записывается cookie.
...
Жёсткая навигация вместо Vue Router - см. [Технические решения](/ru/development/tech-decisions).
...
- [Синхронизация](/ru/development/sync) - как PowerSync синхронизирует данные между сервером и локальной SQLite
- [Офлайн-first](/ru/development/offline-queue) - офлайн-записи, очередь загрузки и обработка ошибок
