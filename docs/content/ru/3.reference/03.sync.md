...
|-------------|---------------|-----------|
| Фатальные (данные / ограничение / RLS) | `22xxx`, `23xxx`, `42501` | Транзакция **отбрасывается** - очередь разблокируется; `setUploadErrorHandler` показывает toast `sync.errors.uploadRejected` |
| Повторяемые (сеть, таймаут) | всё остальное | Ошибка **пробрасывается** - PowerSync повторяет попытку с backoff |

Отбрасывание фатальной операции означает тихое расхождение локальной SQLite и сервера для этой строки. Пользователь получает уведомление через toast и может исправить данные вручную.
...
`waitForFirstSync(timeoutMs=10000)` (`app/services/powersync/db.ts`) разрешается после полной передачи начального серверного состояния в локальную SQLite (или по таймауту). `useInitApp` ожидает этого перед показом основного UI, гарантируя, что пользователь не увидит пустое приложение после входа.
...
- [Офлайн-first](/ru/development/offline-queue) - офлайн-записи, очередь загрузки и обработка фатальных ошибок
