---
title: На Convex
description: Что изменилось между v6 (Firebase) и v7 (Convex). Convex впоследствии заменён Supabase + PowerSync.
navigation:
  icon: i-lucide-hamburger
seo:
  title: Миграция на Convex - что изменилось
  description: Подробное сравнение Finapp v6 (Firebase) и v7 (Convex) - изменения бэкенда, производительность, безопасность, тесты, рефакторинг и архитектура. Историческая справка.
---

> **Три эпохи.** Финапка прошла через три поколения бэкенда:
> - **v6** - Firebase Realtime Database + Firebase Auth
> - **v7** - Convex (backend-as-code) + Better Auth (описано в этом документе)
> - **текущий** - Supabase Postgres + PowerSync (offline-first) + Supabase Auth
>
> Этот документ - историческое сравнение миграции с Firebase на Convex. Улучшения на уровне приложения (производительность, рефакторинг, UX, TypeScript, i18n), описанные здесь, по-прежнему актуальны и присутствуют в текущей кодовой базе. Разделы бэкенда, авторизации, офлайн-очереди и зависимостей отражают состояние v7 Convex; см. [Архитектуру](/ru/development/architecture) для текущей архитектуры Supabase + PowerSync.

Сравнение конечного состояния приложения на `firebase`-ветке (v6, Firebase) и в эпоху Convex (v7).

## Бэкенд

| | firebase (Firebase) | v7 (Convex) |
|---|---|---|
...
| **Миграция** | N/A | `utils/migrate/firebase.js` - CLI для импорта JSON-экспортов Firebase в Convex |
| **Удаление аккаунта** | N/A | `deleteUserDataFull()` - пагинированная очистка таблиц приложения + авторизации |

### Что давал Convex

- **Типизированные мутации** - полный TypeScript на обеих сторонах, автогенерация API-типов
- **Серверная валидация** - лимиты длины строк, диапазоны чисел, непустота полей, бизнес-правила
...
- **Дельта-синхронизация** - после начальной загрузки запрашиваются только изменённые записи

## Паттерн сторов

| | firebase | v7 (Convex) |
|---|---|---|
| **Источник данных** | Firebase listener -> `setTrns()` | Convex query -> `mergeOfflineOps()` -> `set()` |
...
| **Система ID** | Firebase push ID | Префикс `local_` -> замена на Convex ID после мутации |
| **Оптимистичный UI** | Частичный (fire-and-forget) | Полный: обновить UI -> push op -> fire mutation -> remap ID при успехе |
| **Верификация синхронизации** | Нет | XOR-хеш (`fnv1a`) ID транзакций; сравнение хешей клиент - сервер при каждой синхронизации |
...
_(Историческая справка - описывает эпоху v7 Convex. Текущая авторизация: Supabase Auth, только email/password. См. [Архитектуру](/ru/development/architecture).)_

| | firebase | v7 (Convex) |
|---|---|---|
| **Провайдер** | Firebase Auth | Better Auth |
| **Плагин** | `nuxt-vuefire` (авто-управление авторизацией) | `plugins/convex.ts` - ручной жизненный цикл |
| **Поток токенов** | Внутри Firebase SDK | `setAuth(fetchToken)` -> `authClient.convex.token()` -> `authReadyPromise` |
...
_(Историческая справка - описывает эпоху v7 Convex. Текущий офлайн: PowerSync управляет синхронизацией автоматически. См. [Архитектуру](/ru/development/architecture).)_
...
| **Операции** | `saveTrnToAddLaterLocal()`, `removeTrnToDeleteLaterLocal()` | `pushSaveOp()`, `pushDeleteOp()` -> универсальные для любой сущности |
...
| **Поэнтитный реплей** | N/A | `replayWalletOps`, `replayCategoryOps`, `replayTrnOps`, `replaySettingsOps` - извлечённые тестируемые функции |
| **Валидация операций** | Нет | `validateOpData()` с Zod-схемами - повреждённые операции логируются и удаляются |
...
_(В v7 Convex была отдельная vitest-конфигурация `convex`, работавшая в edge-runtime. Текущие тесты выполняются в проектах `unit` (node) и `store` (happy-dom) с моками PowerSync/Supabase.)_
...
_(Историческая справка - описывает изменения от firebase к v7 Convex. Текущие зависимости - в `package.json` проекта.)_

| Удалены (эпоха firebase) | Добавлены (эпоха Convex) |
|---------|-----------|
| `firebase` | `convex` |
...
Зависимости эпохи Convex (`convex`, `@convex-dev/better-auth`, `better-auth`, `convex-helpers`, `convex-test`) впоследствии удалены при миграции бэкенда на Supabase + PowerSync. Добавлены `@powersync/web` и `@supabase/supabase-js`. `zod`, `es-toolkit` и `@vueuse/nuxt` остались.
