...
- **Id переиспользуются, не ремапятся** - id-колонки в Supabase это обычный `text` без FK-ограничений, поэтому скрипт сохраняет исходные ключи Firebase как id; ссылки `parentId` / `walletId` / `categoryId` остаются валидными без отдельного прохода ремаппинга. (Это главное отличие от старого скрипта Firebase -> Convex, которому приходилось ремапить сгенерированные сервером id.)
