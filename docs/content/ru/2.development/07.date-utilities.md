...
| Файл | Назначение |
|------|------------|
| `app/utils/date/period.ts` | Базовые утилиты: `toDuration`, `getStartOf`, `getEndOf` |
| `app/utils/date/types.ts` | Типы: `Period`, `Range` |
| `app/utils/date/civil.ts` | Модель «гражданского дня»: дата транзакции - epoch полуночи UTC, вся математика границ идёт в UTC через `@date-fns/utc` |
| `app/utils/date/labels.ts` | Человекочитаемые подписи диапазонов (`createRangeFormatter`: "Сегодня", "Последние 3 месяца", "Мар - Июн") и `formatTrnDateLabel` |
| `app/app/components/stat/date/types.ts` | Типы: `StatDateParams`, `IntervalsInRangeProps` |
...
| `app/app/components/stat/date/useStatDate.ts` | Composable: реактивное состояние дат с персистенцией в localStorage |
| `app/app/components/stat/intervals.ts` | Распределение транзакций по интервалам (`bucketTrnsByIntervals`), средние значения (`computeAverageTotal`) |
...
`computeDateRange(params, maxRange, now)` преобразует `StatDateParams` в конкретный `{ start, end }`:
...
`useStatDate` оборачивает его в Vue `computed` и передаёт `Date.now()` как `now`. Поскольку `Date.now()` не является реактивной зависимостью, диапазон обновляется только при изменении `params` или `maxRange` (не автоматически в полночь). Обновление страницы или любое изменение параметров запускает перевычисление.
...
```
range: 1 мар - 31 мар, granularityBy: 'week'
...
`createRangeFormatter()` (`app/utils/date/labels.ts`) создаёт человекочитаемые подписи. Каждая функция форматирования возвращает полную строку (без разделения на "начало"/"конец" с последующей конкатенацией):
