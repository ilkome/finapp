...
Nuxt SPA хостится статически (выше описан Vercel, но подойдёт любой статический хостинг, отдающий `app/.output/public` после `pnpm build:app`). Бэкенд-сервисы отдельные:
...
### Push-уведомления (`send-push`)

Web Push требует одну пару VAPID-ключей на проект - публичная половина уходит клиенту, приватная остаётся секретом функции. Как устроена доставка - в [push-уведомлениях](/ru/reference/push-notifications).

1. **Сгенерируйте пару ключей** (пропустите, если у проекта она уже есть - перегенерация инвалидирует все сохранённые подписки в `push_subscriptions`):

   ```bash [Terminal]
   npx web-push generate-vapid-keys --json
   ```

2. **Секреты функции** и деплой:
...
3. **Ключ клиента.** Положите тот же публичный ключ в `VITE_VAPID_PUBLIC_KEY` этого окружения (env в Vercel + локальные `.env.*`), иначе устройства подпишутся под ключом, которым отправитель не сможет подписать сообщение.

По расписанию `send-push` пока никто не вызывает - это примитив доставки для будущих напоминаний.
...
Пять таблиц - пер-юзерные: `categories`, `wallets`, `trns`, `user_settings`, `stat_views`. `rates` глобальная - **не копируем** (её наполняет `fetch-rates`). Копия идёт Postgres -> Postgres (схема между окружениями идентична), с переписыванием `userId` на auth-uid целевого проекта.

::callout{icon="i-lucide-info" color="warning"}
Целевой `userId` - это auth-uid пользователя **в новом проекте**, который появляется только **после первого входа** там. Поэтому: завершите [настройку Google](#вход-через-google-продакшен), сделайте первый вход (подойдёт preview-деплой), считайте uid и тогда импортируйте.
::
